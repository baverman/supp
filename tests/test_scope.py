...
def test_for_without_break():
    ...
    assert nvalues(names_at(scope, p[2])) == {
        'a': {'listitem', 'undefined'},
        'b': {10, 20},
        'c': 10,
    }
    assert nvalues(names_at(scope, p[3])) == {
        'a': {'listitem', 'undefined'},
        'b': {10, 20},
        'c': 10,
    }
    ...
    scope, p = csp("""\
        def foo():
            ...
            if True:
                ...
                b = 10
    """)
    assert nvalues(names_at(scope, p[0])) == {'a': {10, 20}, 'b': 20, 'boo': 'func', 'foo': 'func'}
...
def test_lambda():
    scope, p = csp("""\
        ...
        f = lambda b: a + |a
    """)
    assert nvalues(names_at(scope, p[0])) == {'a': 10, 'b': 'lambda.arg', 'f': 'lambda'}
