...
class extract_visitor(NodeVisitor):
    def process(self, tree: ast.AST, flow: Flow) -> Flow:
        self.top = flow.scope.top
        self.flow = flow
        self.generic_visit(tree)
        return flow

    def make_flow(self, hint: str, parents: list[Flow]) -> Flow:
        return self.top.add_flow(Flow(hint, self.flow.scope, parents))  # type: ignore[arg-type]

    def visit_in_flow(self, nodes: list[t.Any] | t.Any, flow: Flow) -> Flow:
        cur = self.flow
        self.flow = flow
        if nodes:
    ...
    def visit_Import(self, node: ast.Import) -> None:
        ...
        for a in node.names:
            ...
            self.flow.add_name(
                ImportedName(name, loc, declared_at, iname, None, qualified=qualified)
            )
